...
~~最近有朋友反馈无法使用本项目读取步数信息，经过排查，是*微信官方限制了无障碍服务*。~~

~~具体表现为使用无障碍服务获取页面信息时，微信会随机返回一些非本页面的内容。~~

~~已有多个不同开发者反馈相同的问题，目前尚未有官方回复，例如：[最近用户反馈在微信上使用无障碍功能进行朗读翻译出现问题，麻烦官方回复下？](https://developers.weixin.qq.com/community/minihome/doc/0000ccda1507787ec3b2ffbf66b000?jumpto=comment&commentid=0004620fb509889841c2ad153660)、[微信8.0.56版本无法使用无障碍服务，无障碍功能无法获取对应控件id](https://developers.weixin.qq.com/community/develop/doc/0006e6d8cc8a880b0e035996361c00)~~

~~可以尝试使用旧版本的微信，但是不保证旧版本微信没有被热更新无障碍服务限制。~~

目前已通过伪装成系统无障碍服务解决了这个问题，现在可以正常使用了。
...
### 1.0.6
- 新增使用 AI 分析数据的示例（仅为本地界面演示，未实际接入）
- 新增支持设置运行步骤间隔时间
- 新增支持设置不显示详细日志
- 导出 CSV 时支持按筛选条件过滤重复数据
- 完善 CSV 导入/导出
- 修复读取历史记录时消息过期终止条件判断错误
- 修复筛选用户名后没有关闭弹窗
